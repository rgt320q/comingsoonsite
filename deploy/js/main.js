...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.gameManager = {
        ...
        showOverlay(message, type = 'info', requiresAck = false, isGameOver = false) {
            ...
            if (overlayAnimation) {
                if (type === 'foul') {
                    ...
                    // Random goal animation (1 of 3 coin-themed celebrations)
                    const animationType = Math.floor(Math.random() * 3) + 1;
                    overlayAnimation.innerHTML = this.getCoinGoalAnimation(animationType);
                }
            }
            ...
            if (type === 'goal') {
                ...
                // Vibrate for goal celebration - more intense
                if (navigator.vibrate) {
                    navigator.vibrate([100, 50, 100, 50, 200, 50, 100, 50, 300]); // Epic celebration pattern
                }
                
                // Screen flash effect
                gameOverlay.style.animation = 'goal-flash 0.5s ease-out';
                setTimeout(() => {
        ...
        getCoinGoalAnimation(type) {
            // Type 1: Coin shoots straight into goal - Direct shot
            if (type === 1) {
                return `
                        ...
                        <div class="soccer-goal">
                            ...
                            <div class="goal-net-pattern">
                                ${Array(8).fill(0).map((_, i) => `
                                    <div class="net-horizontal" style="--i: ${i}"></div>
                                `).join('')}
                                ${Array(12).fill(0).map((_, i) => `
                                    <div class="net-vertical" style="--i: ${i}"></div>
                                `).join('')}
                            </div>
                            <div class="goal-back-shadow"></div>
                        </div>
                        <div class="shooting-coin">🪙</div>
                        <div class="coin-trail">
                            <span style="--delay: 0s">✨</span>
                            <span style="--delay: 0.05s">✨</span>
                            <span style="--delay: 0.1s">✨</span>
                            <span style="--delay: 0.15s">✨</span>
            ...
            // Type 2: Coin bounces off post into goal - Curved shot
            if (type === 2) {
                return `
                    <div class="coin-goal-animation coin-goal-type-2">
                        ...
                        <div class="bouncing-coin">🪙</div>
                        <div class="bounce-effect">💥</div>
                        <div class="goal-word">G⚽⚽⚽L!</div>
                        <div class="excited-player">
                            <div class="player-jumping">🙌</div>
                            <div class="joy-text">SÜPER GOL!</div>
                        </div>
                    </div>
                `;
            }
            
            // Type 3: Coin spins and enters top corner - Top corner goal
            if (type === 3) {
                return `
                    <div class="coin-goal-animation coin-goal-type-3">
                        <div class="soccer-goal large">
                            ...
                            <div class="corner-impact">
                                ${Array(8).fill(0).map((_, i) => `
                                    <span class="impact-particle" style="--angle: ${i * 45}deg">💫</span>
                                `).join('')}
                            </div>
                        </div>
                        <div class="spinning-coin-curved">🪙</div>
                        <div class="coin-spin-trail"></div>
                        <div class="victorious-player">
                            <div class="player-celebrating">🎉</div>
                            <div class="victory-text">MUHTEŞEM GOL!</div>
                        </div>
        ...
        startFireworks() {
            ...
            const createParticle = (x, y, hue, isSpecial = false) => {
                const angle = Math.random() * Math.PI * 2;
                const speed = Math.random() * (isSpecial ? 15 : 10) + (isSpecial ? 8 : 5);
                return {
                    x: x,
                    y: y,
                    ...
                    color: `hsl(${hue}, 100%, ${isSpecial ? 70 : 50}%)`,
                    size: isSpecial ? 3 : 2,
                    trail: [] // Add trail effect
                };
            };

            const launchFirework = (isSpecial = false) => {
                const x = Math.random() * this.fireworksCanvas.width;
                const y = this.fireworksCanvas.height; // Launch from bottom
                const initialVx = (Math.random() - 0.5) * (isSpecial ? 15 : 10);
                const initialVy = -(Math.random() * (isSpecial ? 20 : 15) + (isSpecial ? 25 : 20)); // Go upwards
                ...
                let rocket = {
                    ...
                    vy: initialVy,
                    alpha: 1,
                    color: `hsl(${fireworkHue}, 100%, ${isSpecial ? 70 : 50}%)`,
                    life: 60, // frames
                    exploded: false,
                    explosionParticles: [],
                    isSpecial: isSpecial,
            ...
            // Launch multiple fireworks immediately for dramatic effect
            launchFirework(true); // Special firework
            setTimeout(() => launchFirework(), 200);
            setTimeout(() => launchFirework(true), 400);
            ...
            const animateFireworks = () => {
                ...
                for (let i = this.fireworks.length - 1; i >= 0; i--) {
                    ...
                    if (!p.exploded) {
                        // Add trail
                        p.trail.push({ x: p.x, y: p.y, alpha: 0.5 });
                        ...
                        if (p.vy >= 0 || p.alpha <= 0.1 || p.life <= 0) { // Explode when starts falling or fades
                            p.exploded = true;
                            const particleCount = p.isSpecial ? 100 : 60; // More particles for special
                            for (let j = 0; j < particleCount; j++) {
                                p.explosionParticles.push(createParticle(p.x, p.y, this.fireworksHue, p.isSpecial));
                            }
                        }
                    } else {
                        for (let j = p.explosionParticles.length - 1; j >= 0; j--) {
                            ...
                            ep.trail.push({ x: ep.x, y: ep.y, alpha: ep.alpha * 0.5 });
                            if (ep.trail.length > 5) ep.trail.shift();
                            
                            ep.x += ep.vx;
                            ep.y += ep.vy;
                            ep.vy += 0.3; // Gravity for explosion particles
                            ep.vx *= 0.98; // Air resistance
                            ep.alpha -= ep.decay;
                    ...
                    for (let t = 0; t < p.trail.length; t++) {
                        ...
                        this.fireworksCtx.globalAlpha = tr.alpha * (t / p.trail.length);
                        this.fireworksCtx.fillStyle = p.color;
                        this.fireworksCtx.beginPath();
                        this.fireworksCtx.arc(tr.x, tr.y, 2, 0, Math.PI * 2);
                    ...
                    this.fireworksCtx.fillStyle = p.color;
                    this.fireworksCtx.shadowBlur = p.isSpecial ? 20 : 10;
                    this.fireworksCtx.shadowColor = p.color;
                    this.fireworksCtx.beginPath();
                    this.fireworksCtx.arc(p.x, p.y, p.isSpecial ? 4 : 3, 0, Math.PI * 2);
                    this.fireworksCtx.fill();
                    this.fireworksCtx.restore();
                    ...
                    if (p.exploded) {
                        for (const ep of p.explosionParticles) {
                            ...
                            for (let t = 0; t < ep.trail.length; t++) {
                                const tr = ep.trail[t];
                                this.fireworksCtx.save();
                                this.fireworksCtx.globalAlpha = tr.alpha * (t / ep.trail.length);
                                this.fireworksCtx.fillStyle = ep.color;
                                this.fireworksCtx.beginPath();
                                this.fireworksCtx.arc(tr.x, tr.y, 1, 0, Math.PI * 2);
                                this.fireworksCtx.fill();
                                this.fireworksCtx.restore();
                            }
                            ...
                            this.fireworksCtx.shadowColor = ep.color;
                            this.fireworksCtx.beginPath();
                            this.fireworksCtx.arc(ep.x, ep.y, ep.size, 0, Math.PI * 2);
                            this.fireworksCtx.fill();
                            this.fireworksCtx.restore();
                        }
                    }
                ...
                // Launch new firework periodically (more frequent)
                if (Math.random() < 0.08) { // Increased frequency
                    launchFirework(Math.random() < 0.3); // 30% chance of special
                }
            };
        ...
        playGoalSound() {
            ...
            try {
                ...
                console.log('Goal sound not available');
            }
        },
        getMousePos(evt) {
